...
abstract class AppSTW {
    const ACCESS_KEY = "500208d9c3858f3";
    const SECRET_KEY = "273c0";

    const THUMBNAIL_URI = "";
    const THUMBNAIL_DIR = "images/stw";
    ...
    public static function getThumbnail($url, $args = null, $force = false) {
        ...
        $path = CUSTOM_MODEL.DIRECTORY_SEPARATOR.self::THUMBNAIL_DIR.$src;
    ...
    public static function getScaledThumbnail($url, $width, $height, $args = null, $force) {
        ...
        if ($force || !file_exists($path) || filemtime($path) <= $cutoff)
            if (($xlg = self::getXLargeThumbnail($url)))
                if (($im = imagecreatefromjpeg(CUSTOM_MODEL.DIRECTORY_SEPARATOR.self::THUMBNAIL_DIR.$xlg))) {
                    list($xw, $xh) = getimagesize(CUSTOM_MODEL.DIRECTORY_SEPARATOR.self::THUMBNAIL_DIR.$xlg);
